...
async fn run_to_completion_returns_final_result() {
    ...
    let result = runtime
        .run_to_completion(
            RunRequest::new("thread-completion", vec![Message::user("hi")]).with_agent_id("agent"),
        )
        .await
        .expect("run should succeed");
...
async fn thread_scoped_state_restores_before_run_start_hooks() {
    ...
    runtime
        .run(
            RunRequest::new("thread-counter", vec![Message::user("first")]).with_agent_id("agent"),
            sink.clone(),
        )
        .await
    ...
    runtime
        .run(
            RunRequest::new("thread-counter", vec![Message::user("second")]).with_agent_id("agent"),
            sink.clone(),
        )
        .await
        .expect("second run should succeed");
...
async fn text_truncation_recovery_continues_on_max_tokens() {
    ...
    let result = runtime
        .run(
            RunRequest::new("thread-text-trunc", vec![Message::user("hi")]).with_agent_id("agent"),
            sink.clone(),
        )
        .await
        .expect("run should succeed");
...
async fn truncation_recovery_gives_up_after_max_retries() {
    ...
    let result = runtime
        .run(
            RunRequest::new("thread-trunc-max", vec![Message::user("hi")]).with_agent_id("agent"),
            sink.clone(),
        )
        .await
        .expect("run should succeed");
