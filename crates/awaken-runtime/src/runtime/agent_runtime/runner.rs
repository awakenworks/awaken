...
mod tests {
        ...
        let runtime = Arc::new(
            AgentRuntime::new(resolver)
                ...
                .with_commit_coordinator(
                    awaken_stores::MemoryCommitCoordinator::wrap(store.clone())
                        as Arc<
                            dyn awaken_contract::contract::commit_coordinator::CommitCoordinator,
                        >,
                ),
        );
        let sink: Arc<dyn EventSink> = Arc::new(NullEventSink);
    ...
    async fn truncation_recovery_continues_on_max_tokens() {
        ...
        let result = runtime
            .run(
                RunActivation::new("thread-trunc", vec![Message::user("hi")])
                    .with_agent_id("agent"),
                sink.clone(),
            )
            .await
