...
use super::compaction::{CompactionResolver, CompactionRuntime, build_compaction_runtime};
...
impl AgentRuntime {
    ...
    async fn run_inner(
        ...
        let background_cancel_managers = super::background_cancellation::managers_for_run(
            &resolved_execution,
            compaction_runtime.as_ref(),
        );
        ...
        let _background_cancel_guard = crate::extensions::background::spawn_run_cancellation_guard(
            run_id.clone(),
            Some(runtime_cancellation_token.clone()),
