...
use awaken_eval::{
    EvalRun, EvalRunItem, MatrixCell, ReplayReport, RuntimeReplayer, expand_cells, mint_run_id,
    replay_all, score,
};
...
/// Above this size the request is rejected with a "use dataset-based
/// run" hint — sync HTTP would otherwise exceed ingress timeouts.
...
pub async fn start_online_eval(
    ...
    if cells.len() > MAX_CELLS_PER_SYNC_ONLINE {
        return Err(ApiError::BadRequest(format!(
            "{} cells exceed sync online cap {}; split or persist as a dataset and use /v1/eval/runs",
            cells.len(),
            MAX_CELLS_PER_SYNC_ONLINE,
        )));
    }

    // Pre-resolve every model so any 404 surfaces before we start
    // burning provider tokens. Tuple = (cell, executor, upstream_model).
    ...
    for cell in cells {
        ...
        resolved.push((cell, executor, upstream_model));
    }
    ...
    // Run cells in parallel with bounded concurrency.
