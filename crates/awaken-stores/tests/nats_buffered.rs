...
async fn checkpoint_buffers_to_nats() {
    ...
    let loaded = ThreadStore::load_messages(&*inner, "nats-t1")
        .await
        .unwrap();
...
async fn flush_persists_to_inner() {
    ...
    let loaded = ThreadStore::load_messages(&*inner, "nats-t2")
        .await
        .unwrap()
        .unwrap();
...
async fn recover_replays_unacked_checkpoints() {
    ...
    assert!(
        ThreadStore::load_messages(&*inner, "nats-t6")
            .await
            .unwrap()
            .is_none()
    );
    ...
    let loaded = ThreadStore::load_messages(&*inner, "nats-t6")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(loaded.len(), 2);
...
async fn multiple_checkpoints_last_wins_on_flush() {
    ...
    let loaded = ThreadStore::load_messages(&*inner, "nats-t7")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(loaded.len(), 2);
...
async fn concurrent_checkpoints() {
    ...
    let loaded = ThreadStore::load_messages(&*inner, "nats-tc")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(loaded.len(), 1);
