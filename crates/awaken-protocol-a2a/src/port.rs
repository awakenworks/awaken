//! The runtime seam the A2A adapter drives (DDD port).
//!
...
//! pending tool, a resume command — with no A2A wire types, so the same host backs
//! every protocol adapter on the same thread.
...
#[async_trait]
pub trait A2aRuntime: Send + Sync {
    ...
    /// The model id echoed in the agent card.
