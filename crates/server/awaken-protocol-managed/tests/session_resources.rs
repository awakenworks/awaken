...
use awaken_protocol_managed::{
    ...
    SessionRealizationControl, SessionRuntime, StepOutcome, ToolPermissionDecision, router,
};
...
#[tokio::test]
/// End-to-end cause graph:
/// C1 compatibility token accepted -> C2 sealed source committed -> C3 exact
/// source revision + Environment Resource holder pinned -> C4 Session baseline
/// committed -> E1 Runtime receives a secret-free exact access pin.
///
/// | Rule | C1 | C2 | C3 | C4 | Result |
/// |---|---|---|---|---|---|
/// | P1 | T | T | T | T | exact pin, no plaintext |
///
/// Negative source/workspace/state rows are generated by the access-compiler
/// decision table below; Runtime pin-shape rows live with Host realization.
    ...
    match &calls[0].resources.inputs[0].source {
        awaken_session_contract::ResolvedInputSource::Repository {
            ...
            let binding = config
                .credential_binding
                .as_deref()
                .expect("Repository config keeps its source binding");
            assert_eq!(credential.access.credential.id, binding);
            assert_eq!(credential.access.credential.revision, 1);
            assert_eq!(
                credential.access.usage,
...
async fn remove_repository_pin_from_retained_row(
    repo: &SqliteManagedSessionRepository,
    session_id: &str,
    key: &str,
) -> awaken_session_contract::SessionRevision {
    let mut retained = repo.get(session_id).await.expect("retained Session row");
    let awaken_session_contract::ResolvedInputSource::Repository { credential, .. } =
        &mut retained.resources.active.inputs[0].source
    else {
        panic!("expected retained Repository input")
    };
    *credential = None;
    let expected_revision = retained.revision;
    let payload = awaken_session_contract::SessionMutationPayload::Replace(retained);
    ...
    match repo
        ...
        .await
        .expect("write retained pre-pin shape")
    {
        awaken_session_contract::SessionMutationResult::Applied { new_revision } => new_revision,
        other => panic!("unexpected retained-row mutation result: {other:?}"),
    }
}

#[tokio::test]
/// Retained-row migration cause graph:
/// C1 bound Repository lacks pin -> C2 Vault compiler installed -> C3 exact
/// source still active/in-Workspace -> E1 root-CAS pin commit before Runtime.
/// An already exact row is a no-op; absent compiler fails without a revision.
///
/// | Rule | C1 | C2 | C3 | Result | Root revision | Runtime I/O |
/// |---|---|---|---|---|---|---|
/// | M1 | T | T | T | exact pin | +1 | none |
/// | M2 | F | - | - | unchanged | same | none |
/// | M3 | T | F | - | reject | same | none |
async fn retained_repository_rows_migrate_through_the_one_pin_compiler() {
    let runtime = AcceptingFake::default();
    let prepared = runtime.prepared.clone();
    let repo = std::sync::Arc::new(
        SqliteManagedSessionRepository::open_in_memory().expect("retained Session repository"),
    );
    let catalog = resource_catalog();
    let vaults = std::sync::Arc::new(awaken_protocol_managed::VaultState::new(
        std::sync::Arc::new(awaken_credential_vault::InMemorySecretStore::new()),
    ...
    let request = serde_json::from_value(json!({
        ...
        "resources": [{
            "type": "github_repository",
            "url": "https://github.com/awaken/retained.git",
            "authorization_token": "retained-migration-secret" // awaken-allow: secret
        }]
    }))
    .unwrap();
    let id = state.create_session(request, None).await.unwrap().id;
    let prepared_after_create = prepared.lock().unwrap().len();

    let legacy_revision =
        remove_repository_pin_from_retained_row(repo.as_ref(), &id, "test:retained:strip-1").await;
    let retained = repo.get(&id).await.unwrap();
    let lease = retained
        .realization
        .clone()
        .expect("created realization lease");
    let begin = awaken_session_contract::BeginSessionRealization {
        session_id: id.clone(),
    ...
    SessionRealizationControl::begin_session_realization(&state, begin.clone())
        .await
        .expect("M1 retained pin migration");
    let migrated = repo.get(&id).await.unwrap();
    assert_eq!(migrated.revision.0, legacy_revision.0 + 1, "M1");
    let awaken_session_contract::ResolvedInputSource::Repository {
        credential: Some(pin),
        ..
    } = &migrated.resources.active.inputs[0].source
    else {
        panic!("M1 must persist the exact pin")
    };
    assert_eq!(pin.access.credential.revision, 1, "M1");
    assert!(
        !serde_json::to_string(&migrated)
            .unwrap()
            .contains("retained-migration-secret"),
        "M1"
    );
    assert_eq!(prepared.lock().unwrap().len(), prepared_after_create, "M1");

    SessionRealizationControl::begin_session_realization(&state, begin.clone())
        .await
        .expect("M2 exact retained row");
    assert_eq!(
        repo.get(&id).await.unwrap().revision,
        migrated.revision,
        "M2"
    );
    assert_eq!(prepared.lock().unwrap().len(), prepared_after_create, "M2");

    let stripped_revision =
        remove_repository_pin_from_retained_row(repo.as_ref(), &id, "test:retained:strip-2").await;
    let state_without_vault = ManagedState::new(runtime)
        .with_session_repo(repo.clone())
        .with_resource_catalog(catalog);
    let error = SessionRealizationControl::begin_session_realization(&state_without_vault, begin)
        .await
        .expect_err("M3 missing compiler must reject");
    assert!(
        error.to_string().contains("configured credential vault"),
        "M3"
    );
    assert_eq!(
        repo.get(&id).await.unwrap().revision,
    ...
    assert_eq!(prepared.lock().unwrap().len(), prepared_after_create, "M3");
}
...
async fn live_repository_credentials_are_references_and_config_versions_are_frozen() {
    ...
    let awaken_session_contract::ResolvedInputSource::Repository {
        ...
        credential: Some(second_credential),
        ..
    ...
    assert_eq!(
        first_credential.access.credential.id,
        first_config.credential_binding.clone().unwrap()
    );
    assert_eq!(
        second_credential.access.credential.id,
        second_config.credential_binding.clone().unwrap()
    );
    assert_ne!(
        first_credential.access.credential,
        second_credential.access.credential
    );
    assert_eq!(
        first_credential.selected_plaintext_holder,
        second_credential.selected_plaintext_holder
    );
