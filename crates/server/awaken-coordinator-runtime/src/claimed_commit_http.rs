...
use awaken_run_ingress::ClaimedCommitRequest;
use awaken_runtime_host::{ClaimedCommitService, respond_host_http};
use awaken_worker_transport_security::{
    VerifiedWorkerContext, authenticate_worker_request, verify_current_worker_identity,
};
...
async fn commit_claimed(
    State(service): State<Arc<ClaimedCommitService>>,
    ...
    respond_host_http(
        service
            .apply_claimed(request)
            .await
            .map(|receipt| serde_json::to_value(receipt).expect("CommitReceipt serializes")),
    )
}
