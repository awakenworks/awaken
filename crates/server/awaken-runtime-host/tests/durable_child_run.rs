...
use awaken_runtime_host::{HostResume, ManagedHost, SharedHost};
...
fn child_run_uses_the_durable_scheduler_and_returns_to_its_parent() {
    // Cause/effect design: C0 the complete production Dispatch Session Runtime
    // composition is installed; C1 the child publication sets write=always_ask;
    // C2 the parent Run awaits delegation; C3 the child Run awaits permission in
    // its own Thread; C4 the caller approves that exact child tool through the
    // parent; C5 the parent claim projection contains no child Thread; C6 the
    // canonical service Runtime drives the composed future. Effects: E0 missing
    // C0 fails closed before claimed execution; E1 expose the child's `write`
    // boundary once; E2 resume that exact child; E3 settle both durable rows; E4
    // commit the child result and parent NaturalEnd reply; E5 avoid a default
    // Tokio-worker stack abort. Constraints: K1 committed Session truth is the
    // sole parent+child read authority; K2 each claim keeps its own projection;
    // K3 the shared queue is the sole execution fence; K4 the parent validates
    // the relationship while the child ticket owns answer kind and call identity.
    // Decision rules: R0=!C0=>E0; R1=C0+C1+C2+C3+C6=>E1+E5;
    // R2=R1+C4=>E2+E3+E4; R3=C5=>use K1, never a stale parent projection.
