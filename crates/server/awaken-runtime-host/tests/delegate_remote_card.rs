...
//! Agent through the neutral [`RemoteAgent`] interface. The wire (which route, how the
...
#[async_trait]
impl RemoteAgent for MockRemote {
    async fn run(
        &self,
        _agent_id: &str,
        _input: &str,
    ...
    async fn card(&self, agent_id: &str) -> Result<Value, DelegationExecutionError> {
        ...
        self.reply.clone().map_err(DelegationExecutionError::new)
    }
}
...
async fn remote_agent_card_fetches_through_the_registered_delegate() {
    ...
    let host =
        SharedHost::new(Arc::new(NoLlm), "test").with_remote_agent("researcher", remote.clone());
...
async fn an_unregistered_remote_is_fail_closed() {
    ...
    let host = SharedHost::new(Arc::new(NoLlm), "test").with_remote_agent("known", remote.clone());
...
async fn a_remote_that_errors_surfaces_an_internal_error() {
    ...
    let host = SharedHost::new(Arc::new(NoLlm), "test").with_remote_agent("flaky", remote.clone());
