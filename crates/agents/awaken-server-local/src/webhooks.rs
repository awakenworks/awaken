//! The webhook plane (ADR-0048 / S10) assembled at the server: the bridge from the
//! managed session lifecycle to the neutral dispatcher, plus the workspace-scoped
//! subscription CRUD surface. This is the ONLY place that knows both the managed
//! `SessionLifecycleSink` port and `awaken-webhook`'s delivery machinery — the wire
//! crate stays webhook-agnostic, the webhook crate stays protocol-neutral.
//!
//! Env-gated: [`webhook_plane`] returns `Some` only when `AWAKEN_WEBHOOK_DIR` is
//! set (durable subscriptions under `webhooks.db`); unset = no webhook plane, the
