...
use awaken_server::{ExtMcpProbe, PreparedMcpRefresh, VaultRefresher, build_management_router_with_model};
...
/// The consumption-side project ingress (ADR-0042 amendment): the same agent
...
async fn project_bindings_give_the_same_agent_different_tool_surfaces() {
    ...
    // Two projects: `tools` binds calc to the agent, `bare` binds nothing.
    for id in ["tools", "bare"] {
        let (s, _) = call(
            &app,
            "PUT",
            &format!("/v1/config/projects/{id}"),
            Some(json!({
                "id": id, "workspace_id": "ws", "display_name": id, "version": 1
            })),
        )
        .await;
        assert_eq!(s, StatusCode::OK);
    ...
    let (s, _) = call(
        &app,
        "PUT",
        "/v1/config/projects/tools/agents/calc-agent/mcp",
        Some(json!({
            "project_id": "tools", "agent_id": "calc-agent",
            "mcp_server_ids": ["calc-def"], "version": 1
        })),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
    let (s, _) = call(
        &app,
        "PUT",
        "/v1/config/projects/bare/agents/calc-agent/mcp",
        Some(json!({
            "project_id": "bare", "agent_id": "calc-agent",
            "mcp_server_ids": [], "version": 1
        })),
    )
    .await;
    assert_eq!(s, StatusCode::OK);

    // Project `tools`: the calc tool is live and the conversation works.
    let (s, session) = call(
        &app,
        "POST",
        "/projects/tools/v1/sessions",
        Some(json!({ "agent": "calc-agent" })),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
    let tools_session = session["id"].as_str().unwrap().to_string();
    let (s, _) = send_user_message(&app, &tools_session, "add 4 5").await;
    assert_eq!(s, StatusCode::OK);
    let events = list_events(&app, &tools_session).await;
    assert!(
        events
            .iter()
            .any(|e| e["type"] == "agent.tool_use" && e["name"] == "mcp__calc__add"),
    ...
    // Project `bare`: same agent id, EMPTY project binding — the model still
    // emits the call (it is deterministic), but no MCP tool is provisioned, so
    // the turn fails loudly instead of silently reaching the server.
    let (s, session) = call(
        &app,
        "POST",
        "/projects/bare/v1/sessions",
        Some(json!({ "agent": "calc-agent" })),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
    let bare_session = session["id"].as_str().unwrap().to_string();
    ...
    assert!(
        !agent_messages(&events)
            .iter()
            .any(|m| m.contains("result: 9")),
        "no successful calc reply on the bare project: {events:?}"
    );

    // An unauthored project 404s at the ingress — before any session exists.
    let (s, body) = call(
        &app,
        "POST",
        "/projects/ghost/v1/sessions",
        Some(json!({ "agent": "calc-agent" })),
    )
    .await;
    assert_eq!(s, StatusCode::NOT_FOUND, "unauthored project: {body:?}");
