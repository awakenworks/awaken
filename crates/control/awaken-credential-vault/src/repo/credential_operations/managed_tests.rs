...
#[tokio::test]
async fn legacy_writing_json_without_attempt_or_writer_fields_is_claimed_and_recovered() {
    let repo = InMemoryCredentialRepo::new();
    let secrets = InMemorySecretStore::new();
    repo.insert_vault("ws", managed_vault()).await.unwrap();
    let command = managed_command("https://mcp.example.com");
    ...
    assert!(legacy.writer_token.is_empty());
    assert_eq!(legacy.writer_epoch, 0);
    assert_eq!(legacy.writer_lease_expires_at_unix_ms, 0);
    assert!(legacy.validate().is_ok());
    ...
    assert_eq!(
        repo.get(&legacy.after_source.id).await.unwrap(),
        legacy.after_source
    );
    assert_eq!(
        repo.get_vault_credential("ws", &child.id).await.unwrap(),
...
#[tokio::test]
async fn stale_writer_cannot_cleanup_material_after_recovery_publishes_it() {
    ...
    pending.writer_lease_expires_at_unix_ms = 100;
    ...
    assert_eq!(
        recover_managed_credential_mutations_at(secrets.as_ref(), repo.as_ref(), 100)
            .await
            .unwrap(),
        1
    );
    let committed = repo
        .get(&CredentialSourceId("source-1".into()))
        .await
        .unwrap();
...
#[tokio::test]
async fn managed_creation_recovery_publishes_complete_and_reclaims_partial_material() {
    let repo = InMemoryCredentialRepo::new();
    let secrets = InMemorySecretStore::new();
    repo.insert_vault("ws", managed_vault()).await.unwrap();
    ...
    let mut complete =
        PendingManagedCredentialMutation::create(source.clone(), child.clone()).unwrap();
    source = complete.after_source.clone();
    complete.writer_lease_expires_at_unix_ms = 200;
    ...
    secrets
        ...
        .unwrap();
    assert_eq!(
        recover_managed_credential_mutations_at(&secrets, &repo, 199)
            .await
            .unwrap(),
        0,
    ...
    assert_eq!(
        recover_managed_credential_mutations_at(&secrets, &repo, 200)
            .await
            .unwrap(),
        1
    );
    ...
    partial.writer_lease_expires_at_unix_ms = 300;
    ...
    secrets
        ...
        .unwrap();
    assert_eq!(
        recover_managed_credential_mutations_at(&secrets, &repo, 300)
            .await
            .unwrap(),
        1
    );
...
async fn in_memory_writing_takeover_rejects_the_stale_owner() {
    ...
    stale_ready.phase = ManagedCredentialMutationPhase::Ready;
...
async fn managed_mutation_batch_continues_after_its_first_item_fails() {
    ...
    for suffix in ["a", "b"] {
        ...
        let mut pending = PendingManagedCredentialMutation::create(source.clone(), child).unwrap();
    ...
    *store.poison.lock().unwrap() = Some(poison);
    ...
    assert!(
        recover_managed_credential_mutations_at(&store, &repo, 100)
            .await
            .is_err(),
...
fn legacy_writing_json_defaults_to_an_expired_unowned_lease() {
    ...
    assert!(decoded.writer_token.is_empty());
    assert_eq!(decoded.writer_epoch, 0);
    assert_eq!(decoded.writer_lease_expires_at_unix_ms, 0);
...
async fn aborted_managed_cleanup_survives_the_crash_prefix_before_delete() {
    ...
    .unwrap();
    assert!(repo.begin_managed_mutation(competing).await.is_err());
...
async fn committed_managed_pair_returns_success_and_retries_failed_cleanup() {
    ...
    assert_eq!(pending[0].phase, ManagedCredentialMutationPhase::Reclaiming);
