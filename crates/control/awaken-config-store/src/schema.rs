...
pub fn config_bundle() -> Result<MigrationBundle, MigrationError> {
    ...
    migrations.push(Migration::per_dialect(
        ...
        // SQLite orders by its built-in `rowid`; nothing to add. A deterministic
        // index on the existing `created_at` column stands in as an inert body;
        // the scoped receipt, not conditional SQL, provides idempotency.
...
mod tests {
    ...
    #[test]
    fn config_bundle_uses_receipts_as_its_only_schema_guard() {
        // Cause/effect decision table: predecessor versions present + receipt
        // absent => deterministic CREATE/ALTER executes; receipt present => the
        // runner skips it; object present without receipt => DDL fails closed.
        // Both dialect bodies must therefore construct without conditional DDL.
