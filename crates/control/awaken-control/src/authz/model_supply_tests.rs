...
fn agent_publisher_can_discover_but_cannot_administer_model_supply() {
    ...
    // binding at one Workspace (C2) + requested action (C3) -> allow only the
    // two capabilities needed by Flow (E1), while credential and model-supply
    // administration remain default-deny (E2).
    //
    // Decision table:
