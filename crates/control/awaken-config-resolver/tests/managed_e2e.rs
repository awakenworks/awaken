//! Management-plane resolution acceptance (ADR-0043) through the public APIs.
...
async fn run_full_chain(
    ...
    model_id: &str,
    secret: &str,
) -> (ExecutableAgentSnapshot, ResolvedInference) {
...
async fn managed_full_chain_resolves_secret_free_snapshot_and_credential() {
    let (snapshot, inference) = run_full_chain(
    ...
    assert_eq!(inference.triple.provider_id, "anthropic");
    assert_eq!(inference.triple.model_id, "claude-opus-4-8");
    assert_eq!(inference.adapter_kind, "anthropic");
    assert_eq!(
...
async fn managed_full_chain_calls_real_model() {
    ...
    let request = ChatRequest {
        model_binding: ModelBinding {
            provider_identity_ref: inference.triple.provider_id.clone(),
