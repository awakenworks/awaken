...
import { Button, Card, EmptyState, Modal, Pill, useConfirm, useToast } from "../components/ui";
...
export default function DreamDetailSurface() {
  ...
  return <div className="stack dream-detail">
    <div className="row" style={{ justifyContent: "space-between", alignItems: "flex-start" }}>
      <div><div className="row"><Button variant="ghost" onClick={() => nav(`/w/${wsId}/memory`)}>← {app.t("Memory", "记忆")}</Button><h2 style={{ margin: 0 }}>{item.id}</h2><DreamStatusPill status={item.status} />{item.archived_at && <Pill tone="neutral">{app.t("archived", "已归档")}</Pill>}<Pill tone="info">{app.t("research preview", "研究预览")}</Pill></div><div className="mut">{new Date(item.created_at).toLocaleString()}{item.ended_at ? ` → ${new Date(item.ended_at).toLocaleString()}` : ""}</div></div>
      <div className="row">
    ...
    <div className="dream-summary-grid">
      <Card><span className="mut">{app.t("Source store", "来源记忆库")}</span><div className="mono">{sourceId}</div>{sourceId && <Button onClick={() => nav(`/w/${wsId}/memory?store=${encodeURIComponent(sourceId)}`)}>{app.t("Open source", "打开来源")}</Button>}</Card>
      <Card><span className="mut">{app.t("Evidence Sessions", "证据会话")}</span><div><strong>{dreamSessionIds(item).length}</strong> {app.t("Sessions", "个会话")}</div><div className="row dream-id-list">{dreamSessionIds(item).slice(0, 8).map((id) => <button className="manage-link mono" key={id} onClick={() => nav(`/w/${wsId}/sessions/${id}`)}>{id}</button>)}</div></Card>
      <Card><span className="mut">{app.t("Model usage", "模型用量")}</span><div className="mono">{item.model.id}</div><div>{tokenTotal(item).toLocaleString()} {app.t("tokens", "个 Token")}</div></Card>
      <Card><span className="mut">{app.t("Output store", "输出记忆库")}</span><div className="mono">{outputId ?? app.t("Not created yet", "尚未创建")}</div>{outputId && <div className="row"><Button variant="primary" onClick={() => nav(`/w/${wsId}/memory?store=${encodeURIComponent(outputId)}`)}>{app.t("Open output", "打开输出")}</Button><Button onClick={() => setUseOutput(true)}>{app.t("Use in Agent", "用于 Agent")}</Button></div>}</Card>
    </div>
    {item.session_id && <Card><div className="row" style={{ justifyContent: "space-between" }}><div><strong>{app.t("Execution Session", "运行会话")}</strong><div className="mono mut">{item.session_id}</div></div><Button onClick={() => nav(`/w/${wsId}/sessions/${item.session_id}`)}>{app.t("Open run details", "打开运行详情")} →</Button></div></Card>}
