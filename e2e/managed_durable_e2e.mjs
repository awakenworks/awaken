...
//
// With SESSION_DEPLOYMENT_INGRESS=durable, a `RunDispatch` is persisted to the
// SQLite `DispatchQueue`, then the
// `DispatchWorker` owns its claimed physical attempt. Direct delivery instead
// uses `DirectAttemptDriver`; both enter the same Runtime commit boundary (G6),
// while only durable delivery owns queue/claim/recovery. A normal echo Run here exercises the whole
...
// The rebuilt Session rehydrates from committed truth and the DispatchWorker runs
// startup recovery against the surviving dispatch store, then a follow-up Run
// completes — proving the dispatch queue is durable and the Worker reconnects to
...
async function main() {
  ...
  // C4=a follow-up Run targets the same Session; C5=success or any earlier
  // failure leaves a running child or a live/disconnected FUSE projection below
  // fixture storage. Effects: E1=one on-disk queue
  ...
  // Constraints/invariant: one persisted dispatch queue owns delivery, the
  // replacement process reuses rather than recreates Session/Run truth, and one
  // outer `finally` owns both server generations, the upstream, and fixture tree.
  let upstream;
  let runningServer;
  try {
