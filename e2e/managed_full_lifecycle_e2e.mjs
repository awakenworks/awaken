...
async function main() {
  ...
  try {
    await withScenarioServer('management', 'mcp', PORT, async (baseUrl, upstream) => {
      ...
      // Memory calls deliberately omit `betas`: every exact SDK exercises its
      // generated endpoint default (legacy Managed beta in 0.105, dedicated
      // Memory beta in later anchors). Supplying the current beta here would
      // combine with the old SDK default and manufacture an invalid request.
      ...
      const memList = await drain(client.beta.memoryStores.memories.list(store.id));
      assert.ok(
