...
|---|---|---|
| **Dreams** (`/v1/dreams`, `dreaming-2026-04-21` header, create/poll/cancel/archive) | Implemented over one CAS lifecycle on the selected SQLite/Postgres Session store. Workspace Dream Agent selection is exposed by the Awaken extension `/v1/dream_agent_configuration`; transient transcript Files are purged at terminal cleanup. | canonical design: [`managed-dream.md`](../docs/design/managed-dream.md); protocol tests: `awaken-protocol-managed/tests/dreams.rs`; Rust cross-module E2E: `awaken-server/tests/managed_dream_e2e.rs`; official SDK E2E: `managed_dream_e2e.ts`; SDK declaration gate: `conformance/sdk_surface_coverage_e2e.mjs` |
