...
families, and runs bidirectional CRUD handoffs for every shared Managed resource,
Native/ACP Session cells, Memory CAS/cursor/subresource negatives, literal HTTP
header framing, and latest-registry Session/Memory runtime smoke. Dreams and Tunnels are explicit 0.117-only
...
| [sessions](https://platform.claude.com/docs/en/managed-agents/sessions) | Agent/env/version binding; 1..50 atomic initial events; override omit/null/replace; body/file/outcome/resource/vault limits; lazy idle | `management_sessions_family_e2e.mjs`, `managed_model_override_e2e.mjs`, `acp_e2e.mjs`, `managed_sdk_runtime_matrix_e2e.mjs`, `managed_resource_api_edge_e2e.mjs` | Decision C + boundary 0/1/50/51 + invalid-batch atomicity + SDK 0.105/0.117 × Native/ACP handoff ✅ |
...
| Memory Store endpoints prefer `agent-memory-2026-07-22`; the Managed-only spelling emitted by supported SDK 0.105 remains a compatible selector. Missing, unknown-only, or both official headers reject before domain work. | memory / beta-headers / reference | `managed_sdk_version_matrix_e2e.mjs`, `managed_sdk_memory_depth_e2e.mjs`, `managed_contract_guard_e2e.mjs` + Memory family/lifecycle suites | decision A3/A4; flat and Workspace paths cover the collection and every subresource; old/new selectors share list/CAS/cursor semantics, including mixed-version contention and removed-row cursors |
