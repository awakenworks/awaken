...
// parks on a mutating tool on the AI-SDK wire and is DENIED on the AG-UI wire. The
...
async function main() {
  await withServer('probe', PORT, async (base) => {
    ...
    assert.ok(parked, `parked on a tool (events: ${s1.events.map((e) => e.type).join(',')})`);
    const toolCallId = parked.toolCallId;
    pass(`parked on AI-SDK write tool (toolCallId=${toolCallId})`);
    ...
    await drain(r2);
    pass('AG-UI carried a deny (ToolMessage.error) for the AI-SDK-parked tool');
    ...
    // The sentinel appears exactly twice — the user message and the PARKED write's
    ...
    pass('cross-protocol deny: parked on AI-SDK, denied on AG-UI, write blocked (no read-back), run still completes');
  });

  console.log('E2E PASS: cross-protocol HITL deny hand-off (AI-SDK park -> AG-UI deny -> blocked + complete).');
}
