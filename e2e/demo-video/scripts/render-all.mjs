...
for (const l of LOCALES) {
  ...
  // Tight teaser cut (GIF_SCENES) under a follow-camera: visible cursor travel,
  // highlight ring, click ripple. Continuous motion encodes at 15fps (12 stutters
  // the pans, 20 bloats the file); 820px keeps it legible and ~6MB for README.
  run(`npx remotion render ${ENTRY} DemoGif "${gifSrc}" ${common}`);
  const filt = 'fps=15,scale=820:-1:flags=lanczos';
  run(`ffmpeg -y -loglevel error -i "${gifSrc}" -vf "${filt},palettegen=max_colors=96:stats_mode=diff" "${palette}"`);
