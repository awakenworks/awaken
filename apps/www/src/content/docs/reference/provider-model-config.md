...
| `adapter_options` | `BTreeMap<String, Value>` | `{}` | Adapter-specific non-secret options. The OpenAI-compatible adapter recognizes `headers` (an object of string→string pairs added as default request headers). `model_discovery_schema` (`"openai"`/`"openai-compatible"`/`"openrouter"` or `"gemini"`/`"google"`) opts a custom adapter into `/models` capability discovery using that schema (see Model capability sources). `model_discovery_auth` (`"bearer"`, `"x-goog-api-key"`, or `"none"`; aliases `"authorization-bearer"`, `"google-api-key"`, `"gemini-api-key"`, `"no-auth"`, and `"disabled"`) overrides only the discovery request auth header; by default OpenAI-compatible discovery uses `Authorization: Bearer` and Gemini discovery uses `x-goog-api-key`. Config validation rejects invalid `model_discovery_schema` / `model_discovery_auth` values. `allow_env_credentials` (boolean) lets a keyless `bearer`/`vertex` provider read its credential from the host environment (e.g. `VERTEX_API_KEY`) — a provider with no `api_key` fails closed unless this is `true`. Unknown keys are accepted by the schema and ignored at build time. Secret *values* must use `api_key`; do not store credential strings here |
...
gateway, opt in with `adapter_options.model_discovery_schema`. Discovery auth is
orthogonal to the adapter: it defaults from the declared discovery schema and can
be overridden with `adapter_options.model_discovery_auth` when a gateway uses a
different header convention. Discovery also reuses non-auth entries from
`adapter_options.headers`, so tenant or routing headers reach `/models`;
auth-like entries from `headers` (`Authorization`, `Proxy-Authorization`,
`Cookie`, `x-goog-api-key`, `X-API-Key`, `api-key`,
`Ocp-Apim-Subscription-Key`, `X-Auth-Token`) are ignored for discovery so they
cannot override `model_discovery_auth` or leak when auth is disabled. Gemini/Google discovery currently backfills token limits only; configure `modalities` and
