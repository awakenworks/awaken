...
`Authorization` and `x-goog-api-key` entries from `headers` are ignored for
discovery so they cannot override `model_discovery_auth` or leak when auth is
disabled. Gemini/Google discovery currently backfills token limits only; configure `modalities` and
