---
title: "简介"
description: "Awaken — Rust 智能体运行时,框架本身就是平台。工具先行,提示词在线调,追踪 / Eval / HITL 与运行时集成。"
---

**Awaken** 是用 Rust 写的生产级 AI 智能体运行时。**框架就是平台**:同一个 runtime 表面承载配置、协议适配、追踪、重放、Eval、权限裁决与管理控制台。涉及存储、密钥或策略的能力由对应模块 / 插件显式接入。
