...
两种模式里，Rust 代码负责可执行能力：`Tool` 实现、插件、provider factory、store、backend factory。托管配置负责 agent 行为：提示词、`model_id`、model pool、允许/排除工具、插件 sections、MCP servers、skills 与权限规则。管理控制台只是 server 模式上的浏览器界面，不替代 runtime 本身。
