---
title: "使用 Generative UI"
description: "当你希望 agent 把声明式 UI 组件发送给前端，而不是只返回文本时，使用本页。"
---
...
Generative UI 给 agent 一个结构化视觉输出通道，而不是把所有交互都压成纯文本。用户需要表单、表格或卡片时，这样更好：前端收到可校验的 UI 文档，agent 仍然走普通 tool、stream 和 permission 流程。
