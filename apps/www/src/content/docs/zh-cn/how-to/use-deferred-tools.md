---
title: "使用延迟加载工具"
description: "当你的代理拥有大量工具，且希望通过在 LLM 需要时才暴露工具 schema 来减少上下文窗口占用时，可以使用此功能。延迟加载工具插件将工具分为 Eager（始终发送）和 Deferred（隐藏直到被请求）两类。LLM 可通过 ToolSearch 工具按需发现延迟加载的工具。"
---
...
- 一个可运行的 awaken 代理运行时（参见 [第一个代理](/awaken/zh-cn/tutorials/first-agent/)）
...
| `crates/awaken-ext-deferred-tools/src/policy.rs` | `DiscBetaEvaluator`；初始分类通过 `DeferredToolsConfig::resolve_mode` 声明式完成 |
