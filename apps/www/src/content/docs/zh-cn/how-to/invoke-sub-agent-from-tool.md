...
fn build_seed(topic: &str, max_sources: u32)
    -> Result<PersistedState, awaken::StateError>
{
...
`ExecutionBackend::capabilities()` 现在返回 `BackendProfile`，用 typed dimensions
...
- **`initial_state_seed` 只对 Local backend 生效。** 只有解析出的 `ExecutionPlan` 是本地执行时才接受。A2A 以及任何尚未实现 seed wire 协议的非本地 backend 都会以 `ExecutionBackendError` 拒绝带 seed 的 delegate 请求，**不会**静默成功。如果远程子真的需要某些数据，请自己把它编码进 prompt。
