---
title: "测试策略"
description: "当你需要测试工具、插件、状态键或完整的代理运行，且不依赖真实 LLM 时，可以使用本指南。"
---
...
完整的可运行示例（含控制台输出）请参见 `crates/awaken/examples/live_test.rs` 和 `crates/awaken/examples/tool_call_live.rs`。
