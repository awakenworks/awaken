...
Effect 是类型化的、fire-and-forget 的副作用事件。和会在 phase 收敛循环内部执行、还能继续级联的 [scheduled actions](/scheduled-actions/) 不同，effect 在 commit 之后才分发，而且 handler 不能再返回新的 `StateCommand`。
...
- [插件系统内部机制](/explanation/plugin-internals/)
