...
2. 需要多副本时,用 [NATS Store](/awaken/zh-cn/how-to/use-nats-stores/) 做分布式 mailbox 与缓冲式 thread 写入。
3. 阅读 [状态键](/awaken/zh-cn/reference/state-keys/) 和 [线程模型](/awaken/zh-cn/reference/thread-model/)，理解状态布局和生命周期。
4. 用 [Shared State](/awaken/zh-cn/how-to/use-shared-state/) 跨 thread 与 agent 类型共享持久状态。
