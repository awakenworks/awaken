---
title: "状态与存储"
description: "开发期决定 Agent 记住什么、恢复什么、共享什么、持久化什么。"
---

State & Storage 是 Build Agents 的第二步：在
[构建 Agent](/awaken/zh-cn/how-to/build-an-agent/) 之后阅读，并在
[服务与集成](/awaken/zh-cn/serve-and-integrate/) 之前完成。它把一个可运行
runtime 变成有明确 memory、recovery 和 distribution 边界的 runtime。

## 目的

状态和存储属于 Build Agents，因为存储选择会在运营前定义 agent 的 runtime
contract。在暴露给运营者前，应先由代码接好这些边界：managed config、mailbox
dispatch、thread state、profile state 和 event history 都依赖已注册的 store。
...
1. 先从 [构建 Agent](/awaken/zh-cn/how-to/build-an-agent/) 开始，确认 runtime、provider、tools 和 plugins。
2. 读 [状态管理](/awaken/zh-cn/explanation/state-management/)，选择 run、thread、shared 或 profile state。
3. 读 [状态与快照模型](/awaken/zh-cn/explanation/state-and-snapshot-model/)，理解插件如何通过 `PhaseContext`、`StateCommand` 和 snapshot mutation 管理状态。
4. 从 [使用文件存储](/awaken/zh-cn/how-to/use-file-store/)、[使用 Postgres 存储](/awaken/zh-cn/how-to/use-postgres-store/) 或 [NATS Store](/awaken/zh-cn/how-to/use-nats-stores/) 确定持久化后端。
5. 用 [Shared State](/awaken/zh-cn/how-to/use-shared-state/) 跨 thread 与 agent 类型共享持久状态。
6. mailbox、config、profile、trace/eval 和 event durability 边界明确后，再进入 [服务与集成](/awaken/zh-cn/serve-and-integrate/)。
