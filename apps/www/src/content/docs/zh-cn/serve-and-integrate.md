---
title: "服务与集成"
description: "把本地 runtime 变成 server 控制面，获得协议适配、持久派发、托管配置、Trace/Eval 与管理控制台。"
---

这条路径面向需要把本地 runtime 暴露给其他系统调用的场景。价值在于一套
agent 实现服务多类客户端：server 模式负责 wire、队列、配置、Trace/Eval 与管理
surface，runtime 仍然是执行核心。
...
1. 阅读 [通过 SSE 暴露 HTTP](/awaken/zh-cn/how-to/expose-http-sse/)，先把 runtime 放到 HTTP 和流式端点后面。
2. 阅读 [集成 AI SDK 前端](/awaken/zh-cn/how-to/integrate-ai-sdk-frontend/)，对接 React + AI SDK v6。
3. 阅读 [集成 CopilotKit（AG-UI）](/awaken/zh-cn/how-to/integrate-copilotkit-ag-ui/)，对接 CopilotKit 前端。
4. 阅读 [使用管理控制台](/awaken/zh-cn/how-to/use-admin-console/)，让操作者通过浏览器调优 agent。
5. 阅读 [部署到生产](/awaken/zh-cn/how-to/deploy-to-production/),加固 server:持久化 store、TLS、密钥、健康探针。
