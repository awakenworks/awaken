...
function generated() {
  ...
  const e2eManifest = JSON.parse(fs.readFileSync(path.join(repoRoot, 'e2e/package.json')));
  assert.equal(
    e2eManifest.dependencies['@anthropic-ai/sdk'],
    currentOracle.version,
    'E2E @anthropic-ai/sdk must be an exact executable mirror of the current oracle anchor',
  );
  const currentWireContract = managedWireContract(currentOracle.module);
  ...
  return {
    oracle: stableJson({
      schema_version: 1,
      current: { ...currentOracle, wire_contract: currentWireContract },
      anchors: summaries,
    ...
    currentWire: `// GENERATED by @awaken/managed-sdk-oracle; do not edit.\n`
      + `export const MANAGED_SESSION_EVENT_TYPES = ${JSON.stringify(currentWireContract.events.outbound, null, 2)} as const;\n`
      + `export const MANAGED_SESSION_PREVIEW_TYPES = ${JSON.stringify(currentWireContract.events.preview, null, 2)} as const;\n`,
  };
}
...
if (command === 'generate') {
  ...
  fs.writeFileSync(path.join(packageRoot, 'src/current-wire.generated.ts'), output.currentWire);
  ...
  assert.equal(
    fs.readFileSync(path.join(packageRoot, 'src/current-wire.generated.ts'), 'utf8'),
    output.currentWire,
    'current Managed wire constants are stale',
  );
