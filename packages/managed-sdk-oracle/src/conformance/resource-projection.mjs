 ...
 * its generated operations. Files and Skills keep beta=true after GA, so the
 * endpoint capability—not an SDK version table—is the authoritative signal.
 */
export function officialBetaResourceProjection(operations, family) {
  ...
  for (const operation of selected) {
    if (operation.transport_query !== BETA_SELECTOR) {
      throw new Error(`${operation.id} must retain the exact ${BETA_SELECTOR} transport selector`);
    }
