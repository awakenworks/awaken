...
- **A-G22: RunResolver validates declared binding fingerprints before execution.** When `ResolutionRequest.expected_binding` is set, `ScopedServerResolver` checks the frozen registry entry's `content_hash` against the declared value before resolving; a mismatch yields `ResolveError::BindingMismatch` or `ResolveError::BindingNotFound`. Enforcer: `validate_binding_fingerprint` in `crates/awaken-server/src/services/frozen_registry.rs`. Validation: `resolver_fails_closed_on_binding_fingerprint_mismatch` and `resolver_fails_closed_when_binding_absent_from_frozen_registry` tests in the same file.
- **A-G28: RunResolver never searches for alternative bindings on fingerprint failure.** A `BindingMismatch` or `BindingNotFound` error is returned immediately; no retry, fallback, or alternative catalog entry is consulted. Enforcer: `validate_binding_fingerprint` returns `Err` immediately without inspecting other registry entries. Validation: same tests as A-G22.
