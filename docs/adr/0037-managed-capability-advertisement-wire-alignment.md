...
- a registered perception member without an override → **omitted** from
  `configs` and inherits the official `always_allow` default
  (`read/glob/grep`);
- a controlled-modification member without an exact override → advertised and
  enforced as `always_ask` (`bash/write/edit`);
- an exact authored confirmation policy →
  `{name, permission_policy:{type:"always_ask"}}`;
- Awaken does not add parallel move/delete or Git-specific Hand tools. File
  rename/delete and Git operations, where a Session exposes them, use the
  generic `bash` member and its whole-call permission decision.
...
membership owner, and its controlled-modification predicate is the sole
default-ask classifier. `server_policy()` and `builtin_hand_tools()` project
those same facts. Neutral `ToolExecutionPolicy` exact overrides remain
authoritative for disabled/always-ask/always-allow configuration; MCP toolsets
independently default to always-ask. There is no second auto-allow literal or
