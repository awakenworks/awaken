...
- Supersedes: the `Done` lifecycle payload that stored a terminal *business
  ...
  (`Pending/Running/Waiting/Completed/Failed/Cancelled`) status enum, and the
...
### D1: One stored authority — the run `Phase`

The committed run fact stores exactly one value for where a run stands:

```text
Phase = Waiting | Ended(EndCause)
```

`Waiting` is a pause; `Ended` carries the single terminal authority. No second
...
(today none does); the stored `Phase` is the only durable value.

### D4: One terminal exit

Every way a run ends — a natural text turn, the step ceiling, a cancellation, a
fault — funnels through a single commit boundary that writes the one `Phase`
...
### D5: A pause is not a terminus

`Waiting` and `Ended` are mutually exclusive by construction: a paused attempt
carries its waiting ticket (committed in the checkpoint's waiting slot, not on
the run fact), and an ended attempt carries its `EndCause`. The engine cannot
produce a phase that disagrees with the presence of a ticket.
...
- The step ceiling is now an honest terminus (`MaxSteps`), not silently reported
  as a natural completion.

## References

- [runtime-behavior.md](../design/runtime-behavior.md) — run lifecycle section and
  role catalog (`Phase` authority, derived projections).
