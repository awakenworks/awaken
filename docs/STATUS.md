...
| Config publication and executable availability | Control and Coordinator | Control persists `StoredPublication` and now registers its immutable snapshot through `ExecutableAgentRegistrar`; `ExecutableAgentCatalog` is the single Coordinator execution projection and AllInOne uses the local adapter; remote/durable registration remains pending; runtime only validates and executes selected immutable data |
