...
| Distributed role E2E | `deploy/k3d/README.md` + `e2e/k3d/distributed_control_e2e.sh` | ADR-0071 verifies production Control and Coordinator composition, signed Worker requests, authenticated registration and Deployment launch, isolated component databases, retry after Coordinator outage, and durable authority restart. The cluster Worker still uses the deterministic Host-executor `WorkerNode` composition; replacing it with the `awaken worker` CLI and combining Credential plus Resource realization remains the final P2-B process-level acceptance item. |
