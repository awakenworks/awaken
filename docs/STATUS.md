...
| Distributed role E2E | `deploy/k3d/README.md` + `e2e/k3d/distributed_control_e2e.sh` | ADR-0071 is verified on a real multi-node K3D cluster through production Control and Coordinator composition, authenticated registration and Deployment launch, database-less remote Workers, isolated component databases, retry after Coordinator outage, and durable recovery after both authority roles restart. The K3D guide is the sole owner of concrete topology and execution details. |
