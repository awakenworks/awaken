...
| Environment definition and executable availability | Control and Coordinator | `awaken-environment-contract` and `EnvRegistry` own static definitions, revisions, and exact sandbox-policy references in Control. `EnvironmentApplication` is the sole command/recovery path; Coordinator owns only `ExecutableEnvironmentCatalog`, `EnvironmentExecutionState`, and WorkQueue. Archive retains Control history and withdraws current execution availability |
