...
| Unified Managed Session Worker execution | Session + Environment WorkQueue + Runtime Host | ADR-0075 is implemented: complete inputs finalize before insertion, every eligible self-hosted Session uses the canonical WorkQueue, Worker Control realizes only frozen truth, and the late-input flag/state/receipt/provisioner/endpoint/example path is removed. Existing root CAS, Resource/MCP generations, realization leases, claim fencing, and attempt decoration remain authoritative. Workspace compilation and decision-table tests are the promotion evidence |
