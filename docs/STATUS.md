...
| `adr/0066-session-service-binding-and-realization.md` | Accepted target; feature implementation gated by contract-closure Slice 0 | Delegated | [remote Worker component catalog](design/remote-worker-protocol.md#10-remote-worker-component-catalog) |
| `adr/0067-credential-custody-model-exposure-and-secret-delivery.md` | Accepted target; feature implementation gated by contract-closure Slice 0 | Delegated | [remote Worker component catalog](design/remote-worker-protocol.md#10-remote-worker-component-catalog) |
...
| Accepted Session MCP and credential-boundary convergence target | Managed/config + Credential + Model + Session + Runtime Host + neutral provisioning boundary | ADR-0066/0067 direction is accepted, but production feature coding is gated by Slice 0. Required closure: ADR-0051 scope envelope alignment; consumed preparation intent and claim-fenced Worker-to-Control application contribution; exact Environment snapshot; replace/tombstone root CAS; canonical Managed MCP full replacement; Session realization lease; sealed payload reference/resolver; atomic dispatch claim-epoch credential binding; exact OAuth refresh/reseal projection; ACP typed last mile. Generic Service, public realizer, automatic LLM Vault authoring, Platform realization, and other transports remain separately gated |
