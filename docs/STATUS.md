...
| Config publication and executable availability | Control and Coordinator | Control persists `StoredPublication` and registers its immutable snapshot through `ExecutableAgentRegistrar`; `ExecutableAgentCatalog` is the single Coordinator execution projection; AllInOne uses `LocalExecutableAgentRegistrar` and split deployment has `HttpExecutableAgentRegistrar` plus the authenticated registration router; durable Coordinator storage and split-role wiring remain pending; runtime only validates and executes selected immutable data |
