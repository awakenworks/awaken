...
| Config publication and executable availability | Control and Coordinator | Control persists `StoredPublication` and registers its immutable snapshot through `ExecutableAgentRegistrar`; `ExecutableAgentCatalog` is the single Coordinator execution projection; AllInOne uses the local adapter, Control uses the HTTP adapter, and Coordinator mounts the authenticated PostgreSQL-backed router; the migration command owns its schema and Worker rejects the token and authority databases; runtime only validates and executes selected immutable data |
| Environment definition and executable availability | Control and Coordinator | `awaken-environment-contract` and `EnvRegistry` own static definitions, revisions, and exact sandbox-policy references in Control. `EnvironmentApplication` registers exact executable facts; Coordinator owns only `ExecutableEnvironmentCatalog`, `EnvironmentExecutionState`, and WorkQueue. Archive retains Control history and withdraws current execution availability |
| Resource application and lifecycle | Resources, currently co-deployed by Coordinator | One `ResourceComponent` selects stores; one `ResourcesApplication` derives `FileApplicationService` and purge scheduling; one router mounts File/Memory/Skill management. Runtime artifact harvesting and public File APIs call the same application port; Worker materialization remains per-kind and claim-fenced |
...
| Accepted Session MCP and credential-boundary convergence target | Managed/config + Credential + Model + Session + Resource + Runtime Host + neutral provisioning boundary | ADR-0066 Slices 1-3 and the in-repository ADR-0067 contracts have landed through committed, causally tested slices: consumed creation intent/application contribution, exact Environment snapshot, replace/tombstone root CAS, canonical dynamic MCP mutation/recovery through one `McpAttachmentRealizer`, exact material resolver, atomic dispatch claim-epoch binding/receipt, typed ACP process-secret last mile, explicit model-exposure policy, exact OAuth refresh, Repository access/holder pins with no bare-source Runtime path, and the neutral Native `PlatformProviderAdapter` contract. Changed production API paths have strict TypeScript E2E line coverage above 95%. G43 remains a target only for an actual authenticated ACP MCP deployment that proves Worker-held substitution and provider-enforced no-bypass networking; built-in providers correctly reject that claim today. Generic Service, public generic realizer, automatic LLM Vault authoring, Platform ACP/MCP realization, and other dynamic transports remain separately gated |
