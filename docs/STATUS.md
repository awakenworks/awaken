...
| Accepted Session MCP and credential-boundary convergence target | Managed/config + Credential + Model + Session + Resource + Runtime Host + neutral provisioning boundary | ADR-0066/0067 are landing through committed, causally tested slices. Implemented closure includes the consumed creation intent/application contribution, exact Environment snapshot, replace/tombstone root CAS, common realization phases, exact material resolver, atomic dispatch claim-epoch binding/receipt, and Repository exact access/holder pin with no bare-source Runtime path. Remaining closure includes canonical dynamic MCP public mutation/recovery, ACP typed last mile, mediated no-bypass evidence, model exposure, and final TypeScript coverage. Generic Service, public generic realizer, automatic LLM Vault authoring, Platform realization, and other dynamic transports remain separately gated |
