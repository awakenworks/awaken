...
| Config publication and executable availability | Control and Coordinator | Control persists `StoredPublication` and registers its immutable snapshot through `ExecutableAgentRegistrar`; `ExecutableAgentCatalog` is the single Coordinator execution projection; AllInOne uses `LocalExecutableAgentRegistrar`, split deployment has `HttpExecutableAgentRegistrar` plus the authenticated router, and `PostgresExecutableAgentRegistrar` persists and replays the same commands; split-role wiring remains pending; runtime only validates and executes selected immutable data |
