...
| [adr/0033-in-process-run-driver.md](../adr/0033-in-process-run-driver.md) | ticket=question / ResumeResult=answer / now_ms=clock; ResumeCommand::from_ticket dedupes resume assembly; Runtime::run_to_completion owns execute→(park→decide→resume)*→end and registers the snapshot, decide is the in-process twin of durable queue delivery; run stays single-shot; dead RunWithSnapshot removed | [index.md](index.md) |
