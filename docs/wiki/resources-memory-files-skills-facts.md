...
- Fact: an explicit release freezes one active writable Repository input, exact branch/canonical lowercase 40-hex commit, and optional canonical expected-prior commit in the Session cleanup operation. Delegated children settle first; the existing Repository realizer creates under an absent lease or updates under that exact prior lease and reobserves after every attempt. The Session root persists exactly one canonical receipt or typed permanent CAS rejection before root cleanup may dispose the working tree; aggregate storage and every Completed/tombstone gate rebind that outcome to the outer Session id. Local and registered-Worker drivers share that root-CAS outcome path. Omitted intent keeps the v1 no-publication path; credentials and mediated capabilities remain effect-local.
- Links: guardrail G48; [ADR-0063 CAS amendment](../adr/0063-resource-input-identity-configuration-pinning-and-lifecycle.md#2026-08-31-amendment-exact-update-lease-and-durable-rejection); [ADR-0075 Worker outcome amendment](../adr/0075-unified-managed-session-worker-execution.md#amendment-2026-08-31-registered-workers-record-the-same-publication-outcome)
- Verification: legacy wire/fingerprint compatibility, lowercase-OID admission, child/publication/root ordering, create/update/replay/response-loss Git decision table, foreign receipt/rejection store rejection, unavailable command retention, permanent rejection replay without a second Git call, and registered-Worker rejection-before-root-cleanup tests.
