...
- Fact: runtime core provides no concrete model-callable tool ids; first-party Hand/Web tools, native `agent_run`, and Managed `list_agents`/`send_message` enter through `awaken-ext-builtin-tools`, which has no generic Task command family.
