...
- Fact: a trusted hosted application creates or monotonically rotates its stable MCP bearer through the existing Credential/Vault WAL and CAS; the current material reference itself fences delayed older generations, while legacy references upgrade from generation zero without a receipt table. The command reads its rollout by the existing primary event id while only the supervisor enumerates the outbox. The application waits for exact existing-target adoption to converge, then gives the returned Vault id to ordinary idempotent Managed Session creation; one exported server-owned helper predicts that Session address without metadata identity, a local mirror, or fallback.
- Links: [ADR-0066](../adr/0066-session-service-binding-and-realization.md#2026-08-24-amendment-hosted-session-identity-and-credential-adoption)
- Verification: idempotent Session prediction/create; positive/zero, exact replay, same-generation conflict, newer rotation, delayed-older rejection, legacy-zero upgrade, concurrent CAS, and no-extra-secret credential rules; no-target/pending/failure/converged adoption, exact acknowledgement, normalized-target, and secret-free response tests.
...
- Fact: hosted governance products create, recover, list, rotate, retire, and validate generic business Credential Resource backing references through Awaken's existing `/v1/config/credentials` owner and Credential repository. An optional descriptor on the same source row owns immutable provider identity, material shape, and exact target+usage contracts; create/material rotation validates shape and every declared usage against the opened material before storage, descriptor-only changes share the expected-version source CAS but cannot change provider, and exact reads remain secret-free. The closed supported matrix is Repository/HTTP Basic, HTTP effect/typed placements, signature verification/exact field algorithms, and Extension/Extension; described Provider/MCP sources remain rejected until their existing compilers carry an exact target. Legacy `provider_id` is accepted only when the descriptor is absent; targetless mounts reject described rows, and every rotation or terminal retirement consumes an expected revision before material changes. One pure Vault compiler projects exact active source facts into access, so protocol and hosted adapters do not duplicate admission; exact-target callers supply the holder, while typed deferred holder selection is confined to undescribed targetless Provider/A2A publication and the dispatch claim remains the exact holder owner. No parallel operation-key map, local Vault, provider-specific catalog, dual-use token shape, issuer, or probe authority is introduced.
- Links: Flow ADR-0041 (resource credential mechanism delegates to the execution secret-by-reference seam).
- Verification: operation-identity create/replay/conflict, purpose/usage support matrix, exact target+usage/source-access admission, descriptor/material create and rotate decision tables, scoped lookup/list, reference-validation, and secret-free response tests.
