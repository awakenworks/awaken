...
- Fact: Vault is storage at rest. Published access separates material source from a recipient-bound sealed payload reference, exact resolver and optional OAuth refresh/reseal access, and lists allowed plaintext-holder trust domains independently from `Forbidden`/`VirtualOnly` model exposure. MCP and Repository Resource generations or the atomic dispatch claim epoch pin purpose-specific exact Environment-requested holders before materialization; claim admission checks frozen Session MCP pins against the selected Worker's installed capabilities through the common admission kernel, and the actual mechanism appears only in a secret-free receipt. Model, MCP, and Repository share one exact material resolver without sharing an aggregate, and Runtime has no bare Repository source-materialization path. There is no custody order, inference Service attachment, runtime holder fallback, or URL/current-Vault refresh rediscovery. Automatic LLM Vault authoring is a separate, not-yet-designed application workflow.
