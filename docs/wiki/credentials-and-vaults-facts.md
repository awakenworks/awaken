...
- Fact: hosted governance products create, recover, list, rotate, retire, and validate generic business Credential Resource backing references through Awaken's existing `/v1/config/credentials` owner and Credential repository. The descriptor on the same source row owns immutable provider identity, material shape, and exact target+usage contracts. Provider, MCP, A2A, Repository, HTTP-effect, signature, and Extension consumers compile their exact target through the one Vault compiler; network/provider access without a target fails closed. MCP URL normalization and the frozen Environment `mcp_holder` are single authorities, and the narrow application-MCP legacy cutover reuses the expected-revision WAL/CAS. Targetless Secret mounts remain legacy-only and reject described rows. No parallel store, selector, holder decision, or compatibility path is introduced.
