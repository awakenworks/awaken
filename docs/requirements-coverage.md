...
| Accepted credential plaintext-boundary and model-exposure target | Credential + Model + Session + dispatch attempt + execution boundary | Credential/Vault + Config publication + Session + Dispatch + Worker/Host + Sandbox providers | [credentials and vaults](design/credentials-and-vaults.md), [ADR-0067](adr/0067-credential-custody-model-exposure-and-secret-delivery.md) | Accepted target, Slice 0 gated: material source and sealed payload reference remain distinct; one exact resolver; exact OAuth refresh/reseal projection; explicit allowed holder trust domains and `Forbidden`/`VirtualOnly` exposure; MCP generation and atomic dispatch claim epoch pin the exact holder/planned realization, with actual mechanism only in a secret-free receipt. Automatic LLM Vault and Platform realization remain separately gated |
