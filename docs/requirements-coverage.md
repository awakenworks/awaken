...
| Direct runtime ingress and durable run ingress | Server boundary | Dispatch / Server | [run ingress and message delivery](design/run-ingress-message-delivery.md), [runtime interface boundaries](design/runtime-interface-boundaries.md) | `DirectRunIngress` stays weak; `DurableRunIngress` adds durability without owning agent truth (realized as a minimal slice in `awaken-run-ingress`, [ADR-0009](adr/0009-durable-run-ingress-slice.md)) |
