...
| Managed Dream Memory Consolidation | Hosted product + Server/Resource/Worker boundary | Managed adapter + Session application + Memory/File data planes + Worker provisioning | [Managed Dream Memory Consolidation](design/managed-dream-memory-consolidation.md) | Public Dream is one durable consolidation job; freeze source/session evidence, export private JSONL, execute an ordinary restricted Agent, mount source read-only and result write-through, and never add another Memory or Agent state truth |
