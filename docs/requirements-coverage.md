...
| Run state and tool-call lifecycle | Core | Runtime Core | [runtime behavior](design/runtime-behavior.md) | Model phases as neutral runtime states; public statuses are projections |
...
| Serializable resolved run input, executable snapshots, executable registration, and fingerprints | Core | Control + Coordinator + Runtime | [configuration-to-response flow](design/config-to-run-execution-flow.md), [publication lifecycle](design/config-publication-lifecycle.md) | Control compiles and persists publications, Coordinator registers rebuildable executable availability, and runtime validates immutable snapshots and builds live objects |
...
| Official builtin tools package | Runtime extension | Runtime Core extension (in-process) | [tool and capability](design/tool-and-capability.md#official-builtin-tools-extension), [D14](design/key-design-decisions.md#d14---concrete-tool-ids-live-outside-runtime-core) | Runtime core provides no concrete tool ids; `awaken-ext-builtin-tools` supplies the six Hand tools, the two Web owners, native `agent_run`, and Managed `list_agents`/`send_to_agent`; it exposes no generic Task command family |
...
| Direct attempts and durable dispatch | Server boundary | Runtime adapter plus Dispatch / Server | [run ingress and message delivery](design/run-ingress-message-delivery.md), [runtime interface boundaries](design/runtime-interface-boundaries.md) | `DirectAttemptDriver` exposes only queue-less attempt operations; `RunDispatch` + `DispatchQueue` + `DispatchWorker` add durability without owning agent truth (historical slice: [ADR-0009](adr/0009-durable-run-ingress-slice.md)) |
| Recoverable remote Worker execution | Server/Neutral platform boundary | Dispatch / Server + Worker plane | [remote Worker protocol](design/remote-worker-protocol.md), [ADR-0065](adr/0065-recoverable-embeddable-remote-worker.md) | Claim is followed by a consistent committed recovery snapshot; Workers commit only through the fenced, versioned, idempotent coordinator protocol and keep no authoritative database |
| Unified Managed Session Worker execution | Managed/config + Session + Environment WorkQueue + Runtime Host | Managed protocol/config + Session + Runtime Host + Worker | [remote Worker protocol](design/remote-worker-protocol.md), [ADR-0066](adr/0066-session-service-binding-and-realization.md), [ADR-0075](adr/0075-unified-managed-session-worker-execution.md) | Implemented: ordinary Managed authoring and metadata replay remain compatible; one profiled command resolves every direct input into revision-1 desired truth with its Frozen or FileResources policy; atomic Applied/Replayed classification lets only a new root realize, activate, and dispatch; canonical commands enforce later Agent, Resource, MCP, and Repository-credential mutation; no post-create completion, replayed external effect, or Worker-authored desired state remains |
| Accepted credential plaintext-boundary and model-exposure target | Credential + Model + Session + Resource + dispatch attempt + execution boundary | Credential/Vault + Config publication + Session + Resource + Dispatch + Worker/Host + Sandbox providers | [credentials and vaults](design/credentials-and-vaults.md), [ADR-0067](adr/0067-credential-custody-model-exposure-and-secret-delivery.md) | Accepted target landing in verified slices: material source and sealed payload reference remain distinct; one exact resolver; exact OAuth refresh/reseal projection; explicit allowed holder trust domains and `Forbidden`/`VirtualOnly` exposure; MCP/Repository generations and atomic dispatch claim epoch pin purpose-specific exact holders before materialization, with actual mechanism only in a secret-free receipt. Automatic LLM Vault and Platform realization remain separately gated |
...
| Session-owned detached execution of ordinary tools | Runtime extension + existing Thread persistence | Background Task extension, Runtime State + Runtime Host observer | [ADR-0076](adr/0076-session-owned-background-tool-execution.md) | Native startup installs the inert plugin and activates its post-commit observer only for snapshots that select it. Claims and completions use ordinary Thread state commands; the observer reuses the canonical tool executor and adds no task-specific service, table, hidden Run, or Runtime dependency on product code. ACP fails before advertisement because its external process cannot expose the same prepared executor. |
...
| On-demand tool discovery | Runtime application | Runtime Core | [tool and capability](design/tool-and-capability.md) | Use the single typed `tool_search`/ToolReference path with provider projection; query/ranking/reveal state remain outside the stable contract |
...
| Managed Dream | Hosted product + Server/Resource/Worker boundary | Managed adapter + Session application + Memory/File data planes + Worker provisioning | [Managed Dream](design/managed-dream.md) | Public Dream is one durable Dream job; freeze source/session evidence, export private JSONL, execute an ordinary restricted Agent, mount source read-only and result write-through, and never add another Memory or Agent state truth |
...
| File, SQLite, PostgreSQL, NATS, and schema management | Server/Neutral Platform | Store adapters / Operations | deployment and storage coverage, [ADR-0012](adr/0012-sqlite-and-postgres-store-backends.md) | Store adapters satisfy the same contracts; backing-service choice does not change domain behavior. PostgreSQL (`awaken-store-postgres`) and SQLite (`awaken-store-sqlite`) commit + dispatch backends share one portable schema; NATS remains a future adapter |
