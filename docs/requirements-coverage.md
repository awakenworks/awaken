...
| Accepted Managed Environment binding and Session MCP lifecycle target | Managed/config + Session boundary | Managed protocol/config + Session + Runtime Host + Provisioning | [remote Worker protocol](design/remote-worker-protocol.md), [ADR-0066](adr/0066-session-service-binding-and-realization.md) | Accepted target, Slice 0 gated: scoped persistence envelope; consumed preparation intent; claim-fenced application contribution; immutable finalized baseline; existing Resource state and one MCP attachment set under replace/tombstone root CAS; canonical Managed full replacement; Session realization lease and exact-generation local/remote stage/publish/drain; frozen `NetworkPolicy` cannot be widened. Implementation evidence remains pending |
