...
| `POST` | `/v1/a2a/message:stream` | 流式发送；未启用 streaming 时返回 `501` |
| `GET` | `/v1/a2a/tasks` | 列出 A2A 任务 |
| `GET` | `/v1/a2a/tasks/:task_id` | 查询任务状态 |
| `POST` | `/v1/a2a/tasks/:task_id:cancel` | 取消任务 |
| `POST` | `/v1/a2a/tasks/:task_id:subscribe` | 订阅任务更新；未启用 streaming 时返回 `501` |
| `POST` | `/v1/a2a/tasks/:task_id/pushNotificationConfigs` | 创建推送通知配置；未启用 push notifications 时返回不支持 |
