...
| G20 | Executor and wait/resume channel values are plain serializable data; no live handle, `Arc<dyn ...>`, channel, or process handle crosses the executor result or resume channel boundaries. | executor result type (`Phase` + `EndCause` + `Failure`) satisfies `Serialize + DeserializeOwned`; durable live-command and wait/resume channel value types will be constrained at their landing | `assert_boundary` serde assertions in `crates/runtime/awaken-runtime-contract/tests/serde_boundary.rs` |
