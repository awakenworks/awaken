...
| `MemoryRepository` | Existing canonical port | Memory data plane | scoped entries, one atomic `snapshot_heads` primitive, CAS, atomic history, redaction, retention hooks | Agent/Session binding and IAM policy; callers must not reconstruct a snapshot with list-plus-read |
