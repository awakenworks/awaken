...
在线文档：[Awaken docs（英文）](https://awakenworks.github.io/awaken) ·
[中文文档](https://awakenworks.github.io/awaken/zh-cn) ·
...
- **流式 LLM 调用能从瞬时故障中恢复。** mid-stream 中断与 idle stall 会被识别，并按四种明确方案恢复：继续文本、回放已完成的 tool call、注入 cancelled tool 提示重启、整轮重启；`Retry-After` 被尊重；`StreamCheckpointStore` 契约让恢复可以跨进程。 ([详情](https://awakenworks.github.io/awaken/zh-cn/how-to/recover-streaming-llms))
...
把 `ConfigStore` 接入 `AppState` 后，可通过 `/v1/config/*` 管理 agents、models、providers 和 MCP servers。参考 [通过配置调优 Agent 行为](https://awakenworks.github.io/awaken/zh-cn/how-to/configure-agent-behavior) 调优 provider、model binding、工具和插件 section。[`apps/admin-console`](./apps/admin-console/) 使用同一套 API，并通过 `VITE_BACKEND_URL` 读取服务端地址。
